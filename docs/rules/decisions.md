...
| `proximity_to_exit` | nieokreślone; wybrana interpretacja | `PE = 1 − dystans_BFS/maks`, 1 na kaflu wyjścia (`upe=normalized`), ta sama wartość w utility person i w terminalu tree policy | Publikacja nie definiuje `PE`. Wszystkie obowiązujące ramiona (UCB1 i ewoluowane) liczone są w tej samej konwencji, więc porównania są spójne. Wcześniejsza kalibracja `PE` binarnego (sekcja niżej) jest etapem historycznym, nie obecną konwencją. |
...
## Obowiązujące konwencje pomiaru

Wyniki w `data/results/` (`ucb1_bfs`, `ours_bfs`, `ours_bfs10`) liczone są
w jednej konwencji, zapisanej w kolumnie `search_policy`:

| Pole | Wartość | Znaczenie |
| --- | --- | --- |
| `upe` | `normalized` | `PE` w użyteczności person: `1 − dystans_BFS/maks`, 1 na wyjściu |
| `pe` | `normalized` | ten sam `PE` jako terminal ewoluowanej tree policy |
| `ic` | `mean3` | `IC` = średnia z `MR`, `PR`, `TR` |
| `fallback` | `visits` | sekwencja awaryjna schodzi po liczbie odwiedzin |

Ewolucja GP i ramię oceniające używają tych samych konwencji. Kalibracja
opisana niżej (`PE` binarne) jest wcześniejszym etapem i uzasadnia wybór
wariantów, nie obecną konwencję.

## Kalibracja: `PE` binarne
...
`binary` jest implementacją `PE` w funkcjach użyteczności person
(`personas.py`), czyli tam, gdzie ta kalibracja obowiązuje. `graded` został
w silniku (`engine.graded_proximity_to_exit`) i jest wybieralny jako
...
| Liczba partii na mapę | nie podaje | jedna, ze stałym seedem (`--seeds-per-map`). Silnik jest deterministyczny, więc para (mapa, seed) wyznacza wynik chromosomu jednoznacznie — dzięki temu ocena elity cache'uje się między generacjami. Cena tej oszczędności: fitness jest wtedy oszacowaniem z sześciu próbek, a mistrz populacji nigdy nie jest przeliczany na nowych ziarnach — w przebiegu kontrolnym przy 10 tys. iteracji `best_fitness` stał identyczny do szóstego miejsca po przecinku przez 13 generacji, przy 80–88 unikalnych chromosomach na 100 (czyli populacja nie skolapsowała, to cache elity). GP może więc selekcjonować pod szczęśliwe losowanie. Kontrola, że to nie zaszkodziło run 0: jego formuła Runnera dała `reached_exit` 0,667 na sześciu mapach treningowych i 67,3% na jedenastu testowych, czyli przeniosła się bez straty. `--seeds-per-map 2` usuwa ryzyko za cenę podwojenia kosztu fitnessu. |
...
`pe_mode` własnej ewolucji jest ciągłe (obecnie `normalized`); to zastrzeżenie
trzeba zaraportować. Pomiar na formule demonstracyjnej
...
w terminalu `PE`, a nie z samego kształtu wyewoluowanego wyrażenia.

Wynik uboczny tamtego demo: najlepsza formuła miała
...
odejściem od artykułu, mimo że jest to standardowy tryb pracy MCTS.

Z tego zapisu wynika własność ważna przy czytaniu każdej tabeli: wygranę zgłasza wyłącznie `_build_tree`, gdy w drzewie powstanie węzeł
...
Artykuł nie mówi, co znaczy „best". `MonteCarloTreeSearch._fallback_sequence`
udostępnia trzy odczytania (konwencja `fallback`, zapisywana w `search_policy`):

- `utility` — ścieżka od korzenia do węzła o najwyższej użyteczności persony
  policzonej na stanie tego węzła (ta sama funkcja eq. 2–5, która ocenia stany
  końcowe symulacji); remisy na korzyść krótszej ścieżki, bo eq. 2–5 karzą
  każdy krok;
- `mean` — zejście zachłanne po `mean_utility` dziecka;
- `visits` — zejście po liczbie wizyt („robust child").

Zejście po `mean_utility` jest wadliwe: węzeł odwiedzony raz, z jednym
szczęśliwym rolloutem, bije węzeł odwiedzony 5000 razy, a taki węzeł jest
liściem, więc marsz urywa się od razu. Zmierzone na 250 partiach awaryjnych:
181 odgrywało dokładnie 1 akcję. Zamiana na `utility` na `map02` przy 20 tys.
iteracji (seedy 0–2): Treasure Collector skarby 0% → 46,7% (artykuł 35%),
obiekty 0% → 32,1% (artykuł 41%), kroki 1 → 287. Win rate bez zmian, zgodnie
z konstrukcją powyżej.
